...
export class ProductVariantAttributeService {
    ...
    async create (data: { productvariantId: number; attributeValueId: number}): Promise<ProductVariantAttribute | null> {
        ...
        const productVariant = await productVariantRepo.findOne({ where: { id: data.productvariantId } });
        const attributeValue = await attributeValueRepo.findOne({ where: { id: data.attributeValueId } });
    ...
    async getAll(): Promise<ProductVariantAttribute[]> {
        return await this.repository.find({ relations: ['productVariant', 'attributeValue'] });
    }


    //get product variant attribute by id
    async getById(id: number): Promise<ProductVariantAttribute | null> {
        return await this.repository.findOne({ where: { id }, relations: ['productVariant', 'attributeValue'] });
    }


    //update product variant attribute
    async update(id: number, data: { productVariantId?: number; attributeValueId?: number}): Promise<ProductVariantAttribute | null> {
